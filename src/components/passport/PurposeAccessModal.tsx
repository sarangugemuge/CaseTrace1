  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Mandatory Purpose Declaration">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/80 p-3 rounded-lg flex items-start gap-3">
          ...
          <div className="text-xs text-amber-900 dark:text-amber-200">
            ...
            <p className="mt-0.5 text-amber-800 dark:text-amber-300/80">
              Document <span className="font-mono font-bold text-slate-900 dark:text-white">{document.name}</span> has classification level{' '}
              <span className="font-mono uppercase font-bold text-amber-700 dark:text-amber-400">{document.sensitivity}</span>. Declared purpose will be immutably recorded in the security audit trail.
            </p>
          </div>
        </div>

        <div>
          <label className="block text-xs font-mono text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">
            Select Official Access Purpose:
          </label>
          <select
            value={selectedPurpose}
            onChange={(e) => setSelectedPurpose(e.target.value as AccessPurpose)}
            className="w-full bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg p-2.5 text-xs text-slate-900 dark:text-slate-200 font-mono focus:border-blue-500 focus:outline-hidden"
          >
            {purposes.map((p) => (
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg p-2.5 text-xs text-slate-900 dark:text-slate-200 focus:border-blue-500 focus:outline-hidden"
          />
        </div>
        ...
        <div className="flex justify-end gap-3 pt-3 border-t border-slate-200 dark:border-slate-800">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-300 rounded text-xs font-mono transition-colors"
          >
            Cancel
          </button>
          <button
